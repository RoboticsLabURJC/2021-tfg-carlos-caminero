...
{
  node_ = rclcpp::Node::make_shared("TurnActionBT");
  publisher_ = node_->create_publisher<geometry_msgs::msg::Twist>("/cmd_vel", 10);

  clock_ = std::make_shared<rclcpp::Clock>(RCL_ROS_TIME);
}

BT::PortsList TurnAction::providedPorts()
{
...
void TurnAction::halt()
{
  ...
  RCLCPP_INFO(node_->get_logger(), "Halt TurnAction");
}
...
BT::NodeStatus TurnAction::tick()
{
  ...
  publisher_->publish(velocity);
  RCLCPP_INFO(node_->get_logger(), "Tick TurnAction");
